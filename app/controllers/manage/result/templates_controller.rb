class Manage::Result::TemplatesController < ApplicationController
  before_action -> { check_admin_status(redirect: true) }
  before_action :set_result_template, only: [:edit, :update]
  ...
  def update
    if @result_template.update(params.require(:result_doc).permit(:content))
      redirect_to edit_manage_result_template_path(@result_template.slug), notice: "Konten surat telah diperbarui"
    end
  end
