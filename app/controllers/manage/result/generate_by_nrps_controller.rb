class Manage::Result::GenerateByNrpsController < ApplicationController
      ...
      if user.is_police?
        ...
        if registrations.any?
          registration_data  = registrations.map do |registration|
            template_status = registration.exam_session.exam.result_doc.content.nil? ? "Belum ada template" : "Template tersedia"

            document_exist = false

            if !registration.score.doc_status.nil?
              if registration.score.doc_processing?
                document_status = "Berkas masih diproses..."
              elsif registration.score.doc_completed?
                document_status = "Berkas sudah selesai"
                document_exist = true
              elsif registration.score.doc_error?
                document_status = "Terjadi Galat!"
              end
            else
              document_status = "Dokumen belum dibuat. Mohon buat dokumen terlebih dahulu dengan menekan tombol UBAH NILAI."
            end

            {
              ...
              template_slug: registration.exam_session.exam.result_doc.slug,
